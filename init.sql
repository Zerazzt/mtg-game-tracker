...
INNER JOIN	`game_participation` AS `gpB`                       -- And all games where "some" Player B played.
       ON 	`gpA`.`game_id` = `gpB`.`game_id`                      -- But only those A/B games that relate to the same game...
      AND 	`gpA`.`player_id` = `playerA`                          -- ...and where "some Player A" is *our* Player A;
      AND 	`gpB`.`player_id` = `playerB`                          -- ...and where "some Player B" is *our* Player B.
...
LEFT JOIN 	`games` AS `won_games`                              -- Next, Grab all of the games...
       ON 	`won_games`.`id` = `gp`.`game_id`                      -- ...that are part of the original list...
      AND 	`won_games`.`winning_player` = `all_players`.`id`      -- ...and that this player has won.
LEFT JOIN 	`games` AS `lost_games`                             -- Lastly, grab all of the games...
       ON 	`lost_games`.`id` = `gp`.`game_id`                     -- ...that are part of the original list...
      AND 	`lost_games`.`winning_player` <> `all_players`.`id`    -- ... and that this player did not win.
